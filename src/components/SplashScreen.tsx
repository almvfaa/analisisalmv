...
const DetailCard: React.FC<{ icon: string; title: string; children: React.ReactNode }> = ({ icon, title, children }) => (
  <div className="flex-1 min-w-[280px] bg-white p-5 rounded-xl shadow-md border border-gray-200/80 hover:shadow-lg hover:border-gray-300 transition-all duration-300">
...
export const SplashScreen: React.FC<SplashScreenProps> = ({ onStart }) => {
  return (
    <div className="relative flex flex-col items-center justify-center min-h-screen bg-gray-50 text-gray-800 p-4 sm:p-8 overflow-hidden fade-in">
      {/* Background Shapes */}
      <div className="absolute top-0 left-0 -translate-x-1/3 -translate-y-1/3 w-96 h-96 bg-teal-200/30 rounded-full filter blur-3xl opacity-50"></div>
      <div className="absolute bottom-0 right-0 translate-x-1/3 translate-y-1/3 w-96 h-96 bg-blue-200/40 rounded-full filter blur-3xl opacity-50"></div>
      ...
      <div className="w-full max-w-5xl text-center z-10">
        ...
        <div className="bg-white/60 backdrop-blur-md border border-gray-200/80 rounded-2xl p-6 md:p-8 mb-12 shadow-sm">
